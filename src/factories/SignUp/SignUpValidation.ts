...
export type ISignUpSchema = z.infer<typeof schema>;
const SignUpSchema = (data: any): ISignUpSchema => schema.parse(data);

export class SignUpValidation implements Validation<ISignUpSchema> {
	...
	validate(input: any): ISignUpSchema | ValidateError {
		try {
			return SignUpSchema(input);
		} catch (error) {
